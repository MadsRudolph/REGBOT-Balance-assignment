...
The balance controller is wired in Simulink so that the gyro-based Lead is combined with pitch \emph{before} the error sum (multiplicative Lead in series). Tested with $\theta_0 = 10^{\circ}$ initial tilt and no external push, the full non-linear model recovers within about one second, and the motor voltage stays well inside the $\pm 9\,\mathrm{V}$ saturation limit.

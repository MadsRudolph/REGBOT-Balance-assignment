...
\paragraph{Simulink implementation.}
The corresponding Simulink wiring of the \texttt{Tilt\_Controller} subsystem is shown in Figure~\ref{fig:simulink_tilt_controller}. The gyro signal is multiplied by \texttt{tdtilt}~$= \tau_d$ and added to pitch, producing the gyro-based ideal Lead $(\tau_d s + 1)\,\theta$ on the feedback path. That signal is subtracted from $\theta_\mathrm{ref}$ (port~3), passed through the sign-flip $-1$, the post-integrator $(\tau_{i,\mathrm{post}} s + 1)/(\tau_{i,\mathrm{post}} s)$, the outer PI $(\tau_i s + 1)/(\tau_i s)$, and scaled by $K_{ptilt}$ to produce the velocity reference at output port~1. Placing the Lead on the feedback path \emph{before} the error sum realises the multiplicative series $(\tau_d s + 1)\,C_{PI}\,C_{PI,\mathrm{post}}$; a parallel add after the PI blocks was tried first and did not produce the intended phase boost at $\omega_c$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/regbot_simulink_tilt_controller.png}
    \caption{Simulink block diagram of the \texttt{Tilt\_Controller} subsystem realising $C_\mathrm{total}(s)$. Inputs: pitch (port~1), gyro (port~2), $\theta_\mathrm{ref}$ (port~3). The gyro path's gain block holds $\tau_d = $ \texttt{tdtilt}; the post-integrator and outer PI are implemented as \texttt{Transfer Fcn} blocks parameterised by \texttt{tipost} and \texttt{titilt}; the final gain is \texttt{Kptilt}. Output port~1 delivers the velocity reference $v_\mathrm{ref}$ to the inner wheel-speed loop.}
    \label{fig:simulink_tilt_controller}
\end{figure}
